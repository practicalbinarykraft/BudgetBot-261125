...
import { usePlannedExpensesScreen, type TabFilter } from "../hooks/usePlannedExpensesScreen";
...
export default function PlannedExpensesScreen() {
  ...
  const {
    ...
    purchaseMutation, cancelMutation, handleDelete, formatDate,
  } = usePlannedExpensesScreen();
  ...
  const renderItem = ({ item }: { item: PlannedTransaction }) => (
    <Card style={styles.card}>
      <CardContent style={styles.cardInner}>
        <View style={styles.topRow}>
          <View style={styles.topLeft}>
            <ThemedText type="bodySm" style={styles.itemName}>{item.name}</ThemedText>
            <ThemedText type="h3" mono color={theme.expense} style={styles.itemAmount}>
              ${item.amount}
            </ThemedText>
          </View>
        ...
        <View style={styles.metaRow}>
          ...
          {item.category ? <Badge label={item.category} variant="outline" /> : null}
        </View>
        {item.status === "planned" ? (
  ...
  return (
    <FlatList
      ...
      refreshControl={
        <RefreshControl refreshing={plannedQuery.isRefetching} onRefresh={() => plannedQuery.refetch()} />
      }
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <View style={styles.headerLeft}>
              <ThemedText type="h2">{"Planned Expenses"}</ThemedText>
              <ThemedText type="bodySm" color={theme.textSecondary}>{"Track upcoming expenses"}</ThemedText>
            </View>
            <Button title="Add" size="sm" onPress={() => navigation.navigate("AddPlannedExpense")} icon={<Feather name="plus" size={14} color={theme.primaryForeground} />} />
          </View>
